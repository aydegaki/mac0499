...
void OnGazeDataEvent(TX_HANDLE hGazeDataBehavior)
{
	...
	if (txGetGazePointDataEventParams(hGazeDataBehavior, &eventParams) == TX_RESULT_OK) {
		...
		queueX[index] = eventParams.X;
		queueY[index] = eventParams.Y;
		float aX = 0;
		float aY = 0;
		...
		//std::sort(queueX, queueX + SIZEQ);
		//std::sort(queueY, queueY + SIZEQ);
		printf("Global: %.1f %.1f\n", aX, aY/SIZEQ);
		index = (index + 1) % SIZEQ;


		move(aX/SIZEQ, aY/SIZEQ);
		//move(queueX[index / 2], queueY[index / 2]);
